    ...
    if form.validate_on_submit(): # Logic to perform after filling in the form
        ...
        if user is not None:
            if user.check_password(form.password.data): # Check if the entered password is correct
                ...
                login_log = LoginLog(user.id, user.user_type.name) # Add the login record to the login logs and update the database
...
def account():
    ...
    profile_image = url_for('static',filename='profile_pics/'+current_user.profile_image) # Get the current profile image of the user
...
def wallet():
    ...
    topups = TopUp.query.filter_by(user_id=current_user.id).order_by(TopUp.time.desc()).paginate(page=page, per_page=20) # Get the topup history in desc order
...
def userrides():
    page = request.args.get('page', 1, type=int) # filter for page number (pagenation)
    transactions = Transaction.query.filter_by(user_id=current_user.id, paid='YES').order_by(Transaction.time.desc()).paginate(page=page, per_page=30) # Get past rides and their transactions
...
def userreviews():
    ...
    reviews = Review.query.filter_by(user_id=current_user.id).order_by(Review.reviewed_at.desc()).paginate(page=page, per_page=30) # Get past ratings and reviews
...
def reportbike():
    ...
    if form.validate_on_submit():  # Logic to perform after filling in the form
        repair = Repair(user_id = current_user.id, bike_number = form.bike_number.data, description = form.description.data, urgency=form.urgency.data) # Create a repair record
...
def user_info(user_id):
    page = [request.args.get('p1', 1, type=int), request.args.get('p2', 1, type=int), request.args.get('p3', 1, type=int), request.args.get('p4', 1, type=int)] # filter for page numbers for multiple tables (pagenation)
